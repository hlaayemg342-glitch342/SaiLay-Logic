...
class SaiLayLogic(App):
    def build(self):
        ...
        self.status_label = Label(text="[ LOGIC VAULT ]\nEnter Key", font_size='18sp', markup=True)
    ...
    def start_trap(self):
        ...
        Clock.schedule_interval(self.hacker_trap, 0.1)

    def hacker_trap(self, dt):
        elapsed = time.time() - self.start_time
        if elapsed < 60:
            logic_rain = "".join(random.choice("01") for _ in range(15))
            self.status_label.text = f"[color=ff0000]ERROR 01\n{logic_rain}[/color]\nTime: {int(60-elapsed)}s"
        else:
            self.status_label.text = "[ LOGIC VAULT ]\nTry Again"
